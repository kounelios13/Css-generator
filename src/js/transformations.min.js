...
(function($, window, documents, exports) {
	...
	var selectEnabledGenerators = (list)=>{
		...
		list.forEach(item =>$(outputs[item]).show());
	};
	var showGeneratorList = ()=>{
		var list;
		...
		var visibleGeneratorsIndex = map.call(visibleGenerators, (i)=>$(i).data("generator"));
		...
		if (len(visibleGeneratorsIndex) == 4) {
			...
			var generatorsToSelect = map.call(visibleGeneratorsIndex,(i)=>".generatorItem[data-index=" + i + "]").join(",");
