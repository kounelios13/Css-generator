...
(function($, window, documents, exports) {
	...
	var decomposer = {
		...
		decomposeBoxShadow: function(code){
			return this.decomposeTextShadow(code);
		},
	...
	$(function() {
		...
		var boxShadowColors = $(".bcolor");
		var boxShadowSliders = $(".box_shadow:not(.bcolor)");
		var radiusSliders = $(".radius");
		...
		$("#addToFavouritesBtn").on("click", function() {
			...
			Array.prototype.slice.call(list).forEach(function(fn) {
		...
		$("#table_box").on("click", "td", function() {
			...
			var colorValues = decomposer.decomposeColor(bshadow);
			log(bshadowValues)
			if(bshadowValues[0]=="inset")
			{
				//Remove the inset value from the list
				bshadowValues.shift();
				//Set inset to 1
				$(boxShadowSliders[4]).val(1);
			}
			//When we remove the inset value from the array(if inset is present)
			//The list must have exactly 4 values.If not replace the missing values
			// with 0's
			log(bshadowValues)
			while(bshadowValues.length < 4)
				bshadowValues.push(0);
			if (len(colorValues) == 3)
				$(boxShadowColors[3]).val(1);

			$.each(boxShadowSliders, function(slider) {
				$(slider).val(bshadowValues.shift());
			});
			$.each(boxShadowColors, function(slider) {
				$(slider).val(colorValues.shift());
			});
			updateText(".box", bshadow);
		});
		$("#table_rad").on("click", "td", function() {
			...
			$.each(radiusSliders, function(index, slider) {
