	...
	function saveCode(type) {
		...
		if(type=="f"){
			...
			if (bshadow == "none" && rad == 0 && final_transformation == "none" && text_shadow == "none"){
				bootbox.alert({title:"Error",message:"There is nothing to save",buttons: {ok: createBtn(`OK ${icons.ok}`,"btn-success")},className:"errorAlert"});
				return;
			}
			file = "div{\n\t";
		...
		try{
			...
			bootbox.alert({title:"An error has occured",message:e.message,className:"errorAlert"});
		}
	}
	function fav(type) {
	...
	var saveToStorage = () => {
		if (!localStorage) {
			...
			if (areAllEmpty(radius_fav, box_fav, text_fav, transform_fav)) {
				bootbox.alert({title: "Error",message: "There is nothing to save",className:"errorAlert"});
				return;
			}
			let app = {
