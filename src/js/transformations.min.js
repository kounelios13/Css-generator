...
(function($, window, document){
	...
	function removefav() {
		...
		if (areAllEmpty(box_fav, radius_fav, text_fav, transform_fav)) {
			...
			var options = {
				...
				buttons: {
					confirm: {
						label: "Delete" + icons.trash,
						className: "btn-danger"
					},
					cancel:{
						label:"Cancel "+icons.ok,
						className:"btn-success"
					}
				},
				callback: (result) => {
		...
		if (!localStorage) {
			...
			if (dataExists) {
				bootbox.confirm({
					...
					buttons: {
						confirm: {
							label: "Override it"+icons.remove,
							className: "btn-warning"
						},
						cancel: {
							label: "Do not override it "+icons.ok,
							className: "btn-info"
						}
					},
					callback: (result) => {
						if (result) {
							...
							bootbox.alert({
								...
								buttons:{ok:{label:"OK "+icons.ok,className:"btn-success"}},
								className: "errorAlert"
							});
						}
					},
			...
			buttons: {
				...
				ok: {
					...
					callback: () => {
						...
						if(!imageNum){
							bootbox.alert({title:"Invalid image",message:"Please select a valid image",className:"errorAlert"});
							return;
						}
	...
	$(function() {
		...
		$("#clearLocal").on("click", function() {
			if (!localStorage || !Lockr.get("appData")){
				bootbox.alert({title:"Error",message:!localStorage?"localStorage is nor supported":"There is nothing to remove",className:"errorAlert"});
				return;
			}
			bootbox.confirm({
				title: "Warning",
				...
				buttons: {
					confirm: {
						label: "Clear localStorage "+icons.trash,
						className: "btn-danger"
					},
					cancel: {
						className: "btn-success"
