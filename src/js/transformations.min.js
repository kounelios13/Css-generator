...
(function($, window, document, exports) {
	...
	var saveToStorage = () => {
		if (!localStorage) {
			...
			if (Lockr.get("appData")) {
				options = {
					...
					callback:(result)=>{
						if(result){
							...
							options = {title:"Override completed",message:"List has been override",className:"successAlert"};
						}
						else{
							options ={title:"Error",message:"List override aborted",className:"errorAlert"};
						}
				...
				Lockr.set("appData", app);
				options = {title:"Success",message:"Favourites saved to localStorage",buttons:{ok:{className:"btn-success"}},className:"successAlert"};
				bootbox.alert(options);
			}
		}
	};
	var checkStorage = () => {
		...
		log(appData.transform);
		log(appData.borderRadius);
		log(appData.boxShadow);
		log(appData.textShadow);
		...
		bootbox.alert({title:"Loaded success",message:"Favourites loaded",className:"successAlert"});
