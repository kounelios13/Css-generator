		...
		$("#clearLocal").on("click",function(){
			...
			bootbox.confirm({
				...
				callback:(result)=>{
					if(result){
						Lockr.flush();
						bootbox.alert({title:"Operation completed",message:"localStorage has been cleared",buttons:{ok:{className:"btn-success"}},className:"successAlert"});
					}
					else{
						bootbox.alert({title:"Operation aborted",message:"localStorage has not been cleared",buttons:{ok:{className:"btn-info"}},className:"errorAlert"});
					}
				},
				className:"warningAlert"
			});
