...
(function($, window, document, exports) {
	...
	var saveToStorage = () => {
		if (!localStorage) {
			...
			var options ={};
			if (Lockr.get("appData")) {
				options = {
					title:"Be carefull",
					message:"There is another list of favourites.Do you want to override it?",
					buttons:{
						confirm:{
							label:"Override it",
					...
					callback:(result)=>{
						if(result){
							...
							options = {title:"Override completed",message:"List has been override"};
						}
						else{
							options ={title:"Error",message:"List override aborted"};
						}
				...
				Lockr.set("appData", app);
				options = {title:"Success",message:"Favourites saved to localStorage",buttons:{ok:{className:"btn-success"}}};
				bootbox.alert(options);
			}
		}
	};
	var checkStorage = () => {
		...
		transform_fav = appData.transform || [];
		radius_fav = appData.borderRadius || [];
		box_fav = appData.boxShadow || [];
		text_fav = appData.textShadow || [];
		transform_fav.forEach(item => {
			final_transformation = item;
			fav("transform");
		});
		log(radius_fav);
		radius_fav.forEach(item => {
			rad = item;
			fav("radius");
		});
		box_fav.forEach(item => {
			bshadow = item;
			fav();
		});
		text_fav.forEach(item => {
			text_shadow = item;
			fav("text-shadow");
		});
		var success = bootbox.alert({title:"Loaded success",message:"Favourites loaded"});
