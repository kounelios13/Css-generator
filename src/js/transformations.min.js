/*jslint node: true */
'use strict';
//Keep decomposer global for easier debugging
var len = (o) => o.length;
var window = {
	jQuery: () => 0
};
...
var decomposer = {
	...
	decomposeTransformation: function(code) {
		...
		var extractSkewValues = function(code) {
			var values = {x:0,y:0};
			string.replace(/\bskew([XY])\s*\(\s*([+-]?[\d.]+)\s*deg\s*\)/g, function(_, axis, value) {
				values[axis.toLowerCase()] = parseFloat(value);
			});
			var array = [];
			for (var value in values)
		...
		var extractTranslateValues = function(code){
			var values = {x:0,y:0,z:0};
...
(function($, window, documents, exports) {
	...
	var Color = function(red, green, blue, alpha) {
		...
		this.b = blue;
		this.a = alpha;
		//If no alpa value was provided treat this color as rgb by setting alpha value to 1
	...
	$(function() {
		...
		$("#table_text").on("click", "td", function() {
			...
			var curSliders;
			var textValues = decomposer.decomposeTextShadow(text_shadow);
			var textColors = decomposer.decomposeColor(text_shadow);
			//No alpha value is provided 
			//so set alpha value equal to 1
			curSliders = sliders.textShadowColors();
			[...curSliders].forEach(colorSlider => {
				var value = textColors.shift();
				$(colorSlider).val(value);
			});
			curSliders = sliders.textShadowSliders();
			[...curSliders].forEach(slider => {
				var value = textValues.shift();
				$(slider).val(value);
			});
			$("#text_shadow_demo").css("text-shadow", text_shadow);
			updateText(".text_shadow_code_output", text_shadow);
		...
		$("#table_box").on("click", "td", function() {
			bshadow = $(this).text();
			var bshadowValues = decomposer.decomposeBoxShadow(bshadow);
			log(bshadowValues)
			var bshadowColors = decomposer.decomposeColor(bshadow);
			[...sliders.boxShadowColors()].forEach(colorSlider => {
				var value = bshadowColors.shift();
				$(colorSlider).val(value);
			});
			[...sliders.boxShadowSliders()].forEach(slider => {
				var value = bshadowValues.shift();
				$(slider).val(value);
			});
		...
		$("#table_rad").on("click", "td", function() {
			rad = $(this).text();
			var radiusValues = decomposer.decomposeRadius(rad);
			[...sliders.borderRadiusSliders()].forEach(slider => {
				$(slider).val(radiusValues.shift());
			});
...
console.log(decomposer.decomposeTransformation("rotateY(2deg) rotateZ(3deg) rotateX(1deg)"))
