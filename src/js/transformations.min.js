	...
	decomposeTransformation: function(code) {
		...
		var extractScaleValues = function(string){
			return string.indexOf("scale(") != -1?
				string.split("scale(")[1].split(")")[0].split(",").map(Number)
				:[0,0];
		};
		...
		//Add explanation
		return [...rotateValues,...scaleValues,...skewValues,...translateValues];
	}
};
(function($, window, documents, exports) {
	...
	$(function() {
		...
		$("#table_tran").on("click", 'td', function() {
			...
			var transformValues = decomposer.decomposeTransformation(final_transformation);
			[...sliders.transformSliders()].forEach(slider=>{
				$(slider).val(transformValues.shift());
			})
