(function($, window, document) {
	...
	var line =Array(4).fill(0); //this array is used to create a new row in each favourite table after adding 3 cells in each row
	...
	function saveCode(type) {
		...
		if(type=="f"){
			...
			if (bshadow == "none" && rad == 0 && final_transformation == "none" && text_shadow == "none"){
				alert({title:"Error",message:"There is nothing to save",buttons: {ok: createBtn(`OK ${icons.ok}`,"btn-success")},className:"errorAlert"});
				return;
			}
			file = "div{\n\t";
		...
		try{
			...
			alert({title:"An error has occured",message:e.message,className:"errorAlert"});
		}
	}
	function fav(type) {
	...
	var saveToStorage = () => {
		if (!localStorage) {
			...
			if (areAllEmpty(radius_fav, box_fav, text_fav, transform_fav)) {
				alert({title: "Error",message: "There is nothing to save",className:"errorAlert"});
				return;
			}
			let app = {
			...
			if (dataExists) {
				confirm({
					...
					callback: (result) => {
						var options = {
							title:result?"Override completed":"Error",
							message:result?"Override completed":"List override aborted",
							buttons: {ok:createBtn(`OK  ${icons.ok}`,"btn-success")},
							className:`${result?'success':'error'}Alert`
						};
						if (result) {
			...
			buttons: {
				...
				ok: {
					...
					callback: () => {
						...
						domCache.get(".bg-image").removeClass("active");
						if (!imageNum) {
							alert({
								title: "Invalid image",
	...
	$(function() {
		...
		$(".navbar-nav a").on("click", function() {
			...
			var num = currentLink.data("panel");
		...
		$("#clearLocal").on("click", function() {
			if (!localStorage || !Lockr.get("appData")) {
				alert({
					title: "Error",
					message: !localStorage ? "localStorage is not supported" : "There is nothing to remove",
					className: "errorAlert"
				});
				return;
			}
			confirm({
				...
				callback: (result) => {
					var options = {
						title:`Operation ${result?'completed':'aborted'}`,
						message:`localStorage has ${!result?'not':''} been cleared`,
						buttons: {ok: createBtn(`OK ${icons.ok}`,"btn-success")},
						className:`${result?'success':'error'}Alert`
					};
					if (result) {
		...
		$("body").on("click", ".list-group-item.generatorItem", function() {
			...
			//don't use domCache.get
			//We need a dynamic reference not an old one pointing to an object that no longer exists
